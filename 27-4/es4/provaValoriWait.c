...
int main ()
{
	...
	{ 	/* figlio */
        ...
        if((scanf("%d", &value))==EOF){
            printf("----------------------!!!il valore riportato non è un numero!!!!!-----------------");
        }
        if(value > 255 || value < 0){
            printf("%d sarà troncato",value);
        }else{
            printf("%d il valore non sarà troncato",value);
        }
		sleep(4); 	/* si simula con un ritardo di 4 secondi che il figlio faccia qualcosa! */
		exit(value);	/* si torna un valore che si suppone possa essere derivante dall'esecuzione di un compito assegnato al figlio */
	}
