...
{   
   ...
   int pidFiglio, status, ritorno;      /* per wait padre */
	...
	{   
		...
		char let[5];
		sprintf(let,"%d",l+1);
        write(piped[1],let,strlen(let)+1);
		printf("DEBUG-Figlio %d scritto %d messaggi sulla pipe\n", getpid(), j);
		write(piped[1], msg, l+1);	/* il figlio che e' il produttore comunica al padre la linea letta dal file dopo averla trasformata in stringa (per il protocollo di comunicazione stabilito con il padre */
		printf("DEBUG-Figlio %d scritto %d messaggi sulla pipe\n", getpid(), j);
